...
def stats_for_dashboard(days=30):
    ...
    try:
        ...
        # OTIMIZAÇÃO CRÍTICA: Execução em conexão isolada e descartável
        top10_sql = text(
            """
            SELECT
                a.sales_office,
        ...
        top10_results = []
        # DESATIVADO TEMPORARIAMENTE: Causa 'idle in transaction' e travamento do banco.
        # Reativar apenas após refatoração completa da tabela movements ou criação de View Materializada específica.
        """
        try:
            # Usa uma conexão RAW direta do engine, ignorando a sessão atual.
            # Isso garante que não haja transações pendentes e que a conexão morra logo após o uso.
            with engine.connect() as conn:
                # Força autocommit para evitar 'idle in transaction'
                conn = conn.execution_options(isolation_level="AUTOCOMMIT")
                top10_results = conn.execute(top10_sql, {"client": client}).fetchall()
        except Exception as e:
            print(f"[WARNING] Top 10 query timed out or failed: {e}")
            top10_results = []
        """
        ...
        return {
            ...
            "good_battery_assets_count": int(
                status_results.good_battery_assets_count or 0
            ),
            "low_battery_assets_count": int(
                status_results.low_battery_assets_count or 0
            ),

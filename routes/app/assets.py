...
def list_assets():
    ...
    try:
        ...
        for row in result:
            ...
            # Transform data: map view columns to expected names
        ...
        for asset in all_assets:
            distance = None
            if user_lat and user_lon and asset.get('latitude') and asset.get('longitude'):
                try:
                    distance = haversine_distance(
                        user_lat, user_lon,
                        float(asset['latitude']), float(asset['longitude'])
...
def search_assets_api():
    ...
    try:
        ...
        for asset in all_filtered_assets:
            ...
            # Temperature filter
            if temperatures:
                if asset_dict["health"] and asset_dict["health"].get('temperature_c') is not None:
        ...
        serial_numbers = [asset_obj.oem_serial_number for asset_obj, _ in paginated_data]

        # Batch query for latest movements
        movement_map = {}
        if serial_numbers:
            latest_movement_subquery = db_session.query(
                func.max(Movement.id).label('max_id')
            ).filter(
                Movement.asset_serial_number.in_(serial_numbers)
            ).group_by(Movement.asset_serial_number).subquery()

            movements = db_session.query(Movement).filter(
...
def get_filter_options_api():
    ...
    try:
        ...
        outlets = [row[0] for row in db_session.query(Asset.outlet).distinct().all() if row[0]]
        cities = [row[0] for row in db_session.query(Asset.city).distinct().all() if row[0]]
        states = [row[0] for row in db_session.query(Asset.state).distinct().all() if row[0]]
        countries = [row[0] for row in db_session.query(Asset.country).distinct().all() if row[0]]
...
def asset_detail(asset_serial_number):
    ...
    - Consumes mv_client_assets_report MATERIALIZED VIEW for all asset data
    ...
    try:
        ...
        from sqlalchemy import text as sql_text
        sql_query = sql_text("""
            SELECT * FROM mv_client_assets_report 
        ...
        # Transform location fields (view uses latest_latitude/latest_longitude)
        lat = asset_dict.pop('latest_latitude', None)
        lon = asset_dict.pop('latest_longitude', None)
        asset_dict['latitude'] = lat
        asset_dict['longitude'] = lon
        asset_dict['has_location'] = bool(lat and lon)

        # Get latest health events (separate query - necessary for charts)
        latest_health_temperature = db_session.query(HealthEvent).filter(
            HealthEvent.asset_serial_number == asset_serial_number,
            HealthEvent.event_type == "Cabinet Temperature"
        ).order_by(HealthEvent.event_time.desc()).first()

        # Get latest any health event
        latest_health_any = db_session.query(HealthEvent).filter(
            HealthEvent.asset_serial_number == asset_serial_number
        ).order_by(HealthEvent.event_time.desc()).first()

        # Get latest movement
        latest_movement = db_session.query(Movement).filter(
        ...
        user_lat, user_lon = get_user_location()
        distance_km = None
        if user_lat and user_lon and asset_dict.get('latitude') and asset_dict.get('longitude'):
            try:
                distance_km = haversine_distance(
                    user_lat, user_lon,
                    float(asset_dict['latitude']), float(asset_dict['longitude'])
                )
        ...
        return render_template(
            ...
            asset=asset_dict,
            latest_health=latest_health_temperature.to_dict() if latest_health_temperature else None,
            latest_health_any=latest_health_any.to_dict() if latest_health_any else None,
...
def get_assets_by_distance_api():
    ...
    try:
        ...
        for item in paginated:
            ...
            distance_value = item['distance']
            asset_dict['distance_km'] = distance_value if math.isfinite(distance_value) else None
...
@assets_bp.route("/api/temperature-history/<string:asset_oem_serial_number>", methods=["GET"])
def get_asset_temperature_history(asset_oem_serial_number):
    ...
    try:
        ...
        asset = db_session.query(Asset).filter(
            Asset.oem_serial_number == asset_oem_serial_number
        ).first()
        ...
        if health_event and asset.latitude and asset.longitude:
            ...
            if asset.outlet_code:
                ...
                outlet = db_session.query(Outlet).filter(
                    Outlet.code == asset.outlet_code
                ).first()
            ...
            asset_dict["health_history"] = {
                ...
                "temperature_f": health_event.temperature_f,
                "event_time": health_event.event_time.isoformat() if health_event.event_time else None,
        ...
        all_health_events = db_session.query(HealthEvent).filter(
            HealthEvent.asset_serial_number == asset_oem_serial_number,
            HealthEvent.event_type == "Cabinet Temperature",
            HealthEvent.temperature_c.isnot(None)
        ).order_by(HealthEvent.event_time.desc()).limit(10).all()
